...
기본 signature palette는 cobalt blue입니다. Checkbox와 RadioGroup의 선택 표시는 각각 `--dds-checkbox-indicator`, `--dds-radio-indicator`로, Switch thumb는 `--dds-switch-thumb-bg`로 조정할 수 있습니다. Tabs의 구분선과 활성 상태는 `--dds-tabs-border`, `--dds-tabs-active-border`, `--dds-tabs-active-text`로 조정합니다. Accordion의 구분선과 Trigger 상태 배경은 `--dds-accordion-border`, `--dds-accordion-trigger-hover-bg`, `--dds-accordion-trigger-open-bg`로 조정합니다. Popover의 surface는 `--dds-popover-bg`, `--dds-popover-border`, `--dds-popover-radius`, `--dds-popover-shadow`로 조정합니다. Toast의 surface와 tone accent는 `--dds-toast-*` token으로 조정합니다. Alert의 inline feedback surface와 tone 상태는 `--dds-alert-*` token으로 조정합니다. Avatar의 background, text, border, radius, size는 `--dds-avatar-*` token으로 조정합니다. Progress의 track, indicator, radius, height는 `--dds-progress-*` token으로 조정합니다. Breadcrumb의 text, link, current page, separator는 `--dds-breadcrumb-*` token으로 조정합니다. Skeleton의 shimmer는 `--dds-skeleton-bg`, `--dds-skeleton-highlight`, `--dds-skeleton-radius`로 조정합니다. Select의 trigger, listbox, item 상태는 `--dds-select-*` token으로 조정합니다. Pagination의 page link와 현재 페이지 상태는 `--dds-pagination-*` token으로 조정합니다. Table의 surface, header, row state는 `--dds-table-*` token으로 조정합니다. EmptyState의 surface, icon, text는 `--dds-empty-state-*` token으로 조정합니다. FilterBar의 surface와 layout container는 `--dds-filter-bar-*` token으로 조정합니다.
